    ...
    class DescriptionGeneratorTest < ActiveSupport::TestCase
      setup do
        ...
        @place_data = {
          ...
          formatted: "Baščaršija, Sarajevo",
          address_line1: "Baščaršija"
        }
      end

      # === Constants ===

      test "LOCALES_PER_BATCH is set to 5" do
        assert_equal 5, Ai::LocationEnricher::DescriptionGenerator::LOCALES_PER_BATCH
      end

      # === generate tests ===

      test "generate returns descriptions hash keyed by locale" do
        stub_openai_queue(descriptions: { "en" => "English description" }) do
          result = @generator.generate(@location, @place_data, locales: [ "en" ])

          assert_kind_of Hash, result
          assert_equal "English description", result["en"]
        end
      end
      ...
      test "generate uses all supported locales when locales not specified" do
        ...
        Locale.stub :ai_supported_codes, supported do
          stub_openai_queue(descriptions: { "en" => "English", "bs" => "Bosanski" }) do
      ...
      test "generate processes locales in batches" do
        locales = %w[en bs hr de es fr it]  # 7 locales, needs 2 batches (5 + 2)
        request_count = 0

        Ai::OpenaiQueue.stub :request, ->(*args) {
          request_count += 1
          { descriptions: locales.to_h { |l| [ l, "Text in #{l}" ] } }
        } do
          @generator.generate(@location, @place_data, locales: locales)

          # Should be 2 batches: first 5, then 2
          assert_equal 2, request_count
        end
      end

      test "generate merges results from multiple batches" do
        locales = %w[en bs hr de es fr]  # 6 locales, needs 2 batches (5 + 1)

        call_count = 0
        Ai::OpenaiQueue.stub :request, ->(*args) {
          call_count += 1
          if call_count == 1
            { descriptions: { "en" => "English", "bs" => "Bosanski", "hr" => "Hrvatski", "de" => "German", "es" => "Spanish" } }
          else
          ...
          assert_equal 6, result.keys.count
      ...
      test "generate skips empty batch results" do
        ...
        Ai::OpenaiQueue.stub :request, ->(*args) {
          call_count += 1
          if call_count == 1
            { descriptions: { "en" => "English", "bs" => "Bosanski", "hr" => "Hrvatski" } }
          else
            {}  # Empty result
          end
        } do
      ...
      test "schema_for includes all provided locales" do
        ...
        assert_includes desc_props.keys, "en"
        assert_includes desc_props.keys, "bs"
        assert_includes desc_props.keys, "hr"
      end
      ...
      test "schema_for requires all locales" do
        ...
        required = schema[:properties][:descriptions][:required]
      ...
      test "location_vars returns correct hash" do
        ...
        assert_equal "Baščaršija, Sarajevo", vars[:address]
      end
      ...
      test "location_vars joins categories with comma" do
        place_data = @place_data.merge(categories: [ "tourism", "attraction", "historical" ])
        vars = @generator.send(:location_vars, @location, place_data)

        assert_equal "tourism, attraction, historical", vars[:categories]
      end

      test "location_vars falls back to location category_name when no categories" do
        @location.define_singleton_method(:category_name) { "Historical Site" }
        vars = @generator.send(:location_vars, @location, {})

        assert_equal "Historical Site", vars[:category]
      end
      ...
      test "generate creates proper API request context" do
        ...
        assert_includes context_used, "LocationEnricher:descriptions"
