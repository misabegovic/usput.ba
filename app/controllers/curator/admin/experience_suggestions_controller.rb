...
module Curator
  module Admin
    # Experience suggestions approval controller for admin users.
    # Allows admins to approve or reject experience suggestions from curators.
      ...
      def approve
        if @suggestion.pending?
          if @suggestion.approve!(current_user, notes: params[:admin_notes])
            record_activity(:approve_suggestion, recordable: @suggestion, metadata: { type: "ExperienceSuggestion" })
            redirect_to curator_admin_suggestions_path,
              notice: "Prijedlog odobren."
          else
            redirect_to curator_admin_experience_suggestion_path(@suggestion),
      ...
      def reject
        if @suggestion.pending?
          if @suggestion.reject!(current_user, notes: params[:admin_notes])
            record_activity(:reject_suggestion, recordable: @suggestion, metadata: { type: "ExperienceSuggestion" })
            redirect_to curator_admin_suggestions_path,
