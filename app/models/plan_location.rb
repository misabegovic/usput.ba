class PlanLocation < ApplicationRecord
  ...
  # Allow same location on different days (unique per plan+location+day via DB index)
  validates :location_id, uniqueness: { scope: [ :plan_id, :day_number ], message: "already exists on this day" }
  ...
  def next_position_for_day(day_num)
    (plan&.plan_locations&.where(day_number: day_num)&.maximum(:position) || 0) + 1
  end
end
